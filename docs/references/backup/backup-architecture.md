...
| `root` | —（手写） | 必填 | — |
| `renamable` | —（手写） | 必填 | — |
| `identityKey` | `primaryKeys[root].columns`（root PK）；**root 有 UNIQUE 约束（非 PK）时 identityKey 须含 UNIQUE 键**（业务对齐，防跨设备同值不同 UUID 撞 SQLite UNIQUE——如 agent_workspace.path / tag.name / note(rootPath,path) / pin(entityType,entityId) / agent_global_skill.folderName / job_schedule(type,name)） | PK 是复合且 alignment 键非全 PK | "natural-key 复合 PK 用单列" |
