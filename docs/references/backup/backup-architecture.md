...
`finalize #1` 要求恰 14 域。下表集中列出（散落信息见 §三.1 精简范围 + §5 各域注意点）。`identityClass` / 默认 `conflictDefault` 为 finalize 派生值（§6.2 派生规则），显式声明仅用于偏离默认。
...
- **agent_session.workspaceId**：session renamable:true、workspaceId→agent_workspace（cascade，renamable:false）。RENAME 克隆 session 共享旧 workspace，删任一可能 cascade 影响另一。实施期评估：agent_session 改 renamable:false（退化 SKIP），或 cloneAggregate 连带克隆 workspace。
- **topic.activeNodeId**：标量 text soft ref（指向 message，无 FK），不在 jsonSoftReferences（只覆盖 JSON blob）。RENAME 克隆 topic 时 activeNodeId 指向旧 message。实施期：cloneAggregate rootRow 处理重写 activeNodeId（映射到新 topic 对应 message），或 jsonSoftReferences 扩展标量列覆盖。

此两条为 RENAME 路径已知边缘 case，不阻塞架构定稿。
...
- #14/#15 派生自 owning references（§6.2 `AggregateBoundary` 派生公式）
